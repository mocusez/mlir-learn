...
public:
  ...
  matchAndRewrite(mlir::Operation *op, mlir::ArrayRef<mlir::Value> operands,
    ...
    auto loc = op->getLoc();
    mlir::Value helloWorld = getOrCreateGlobalString(
        loc, rewriter, "test_string",
        mlir::StringRef("Testing, Testing! \n\0", 16), parentModule);
...
public:
  explicit AddOpLowering(mlir::MLIRContext *ctx)
      : mlir::ConversionPattern(hello::AddOp::getOperationName(), 1, ctx) {}

  mlir::LogicalResult
  matchAndRewrite(mlir::Operation *op, mlir::ArrayRef<mlir::Value> operands,
                  mlir::ConversionPatternRewriter &rewriter) const override {
    hello::AddOpAdaptor adaptor(operands);

    // Get operands
    auto lhs = adaptor.getLhs();
    auto rhs = adaptor.getRhs();

    // Create LLVM add operation
    auto result = rewriter.create<mlir::LLVM::AddOp>(op->getLoc(), lhs, rhs);
...
void HelloToLLVMLoweringPass::runOnOperation() {
  ...
  patterns.add<hello::PrintOpLowering, hello::WorldOpLowering, hello::TestOpLowering,hello::AddOpLowering>(&getContext());
